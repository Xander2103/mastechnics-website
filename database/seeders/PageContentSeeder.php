...
/**
 * Test/dev content layer used by ServicePageTest on top of PageSeeder.
 *
 * NOT for production: the meta titles/descriptions below predate the local
 * SEO rewrite (migration 2026_08_04_000003) and would overwrite the
 * live-intent meta of every service page. Older sprint plans still mention
 * running it on the server — that instruction is obsolete.
 */
    ...
    public function run(): void
    {
        if (app()->isProduction()) {
            throw new \RuntimeException(
                'PageContentSeeder is a test/dev content layer and must never run in production: it would overwrite the local SEO meta.'
            );
        }
